...
## 2.5 편집하면서 보기 — `clotho dev`

문서를 고치고 결과를 확인하는 데 GIF를 다시 굽지 않아도 된다.
...
디렉터리의 `*.json`을 목록으로 띄우고, 파일이 밖에서 바뀌면 **재생 위치를 유지한 채** 다시 그린다. 검증(`validate`)과 린트(`lint`) 결과를 같은 화면에 띄우므로 저장하고 터미널로 옮겨가지 않아도 된다.

에디터를 대체하지 않는다. `@kokoa/clotho-editor`는 문서를 만드는 UI이고, 이쪽은 **git 작업 트리의 파일이 진실인 루프**를 위한 것이다 — 브라우저는 임의의 로컬 디렉터리에 접근할 수 없어서 에디터 혼자서는 이 자리를 채울 수 없다.

> [!WARNING]
> 브라우저가 보낸 내용을 파일로 쓰는 엔드포인트가 있으므로 기본 바인딩이 `127.0.0.1`이다. 신뢰할 수 없는 네트워크에 노출하지 말 것.

## 2.6 무엇이 바뀌었는지 보기 — `clotho diff`

JSON 텍스트 diff로는 애니메이션 문서의 변경을 읽을 수 없다. 요소 하나를 추가하면 수백 줄이 움직이고, 키프레임 배열은 통째로 재작성된 것처럼 보인다.
...
저작자의 단위로 보고한다 — 요소가 움직였다, 트랙에 키프레임이 늘었다, 챕터가 밀렸다. `cell-0`을 `slot-0`으로 바꾼 것처럼 id만 달라진 경우는 **삭제+추가가 아니라 rename으로 추정해서** 보여주고(추정임을 표시한다), 2px 미만의 이동이나 부동소수 마지막 자리는 기본적으로 접는다.
...
## 2.8 독자가 속도를 쥐게 하기 — 스크롤리텔링

긴 글 속의 애니메이션은 대부분 읽히지 않는다. 재생 버튼을 누르지 않고, 눌러도 자기 읽기 속도와 무관하게 흘러가는 화면을 따라가지 못한다. 자동 재생은 반대 문제를 만든다 — 앞 문단을 읽는 중인데 화면이 이미 끝나 있다.

`mountScrollPlayer`는 스크롤 위치를 시각으로 쓴다. `snapToChapters`를 켜면 **챕터마다 같은 양의 스크롤**을 배분하므로, 200ms 만에 일어나지만 중요한 순간이 몇 픽셀로 지나가지 않는다.
