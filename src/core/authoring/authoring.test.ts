...
describe('authoring helpers', () => {
  it('returns a fully parsed, JSON-compatible document', () => {
    const doc = defineAnimation({
      ...
      duration: 1000,
      elements: [
        { type: 'circle', id: 'dot', cx: 10, cy: 10, r: 5, appearances: [appear(0, 1000)] },
      ],
  ...
  it('expands repeat and stagger patterns to explicit JSON data', () => {
    expect(
      repeatAppearances({ count: 3, start: 100, duration: 200, gap: 50 }).map((a) => [
        a.start,
        a.end,
      ]),
    ).toEqual([
    ...
    expect(
      track('cx', [
        { time: 0, value: 0 },
        { time: 100, value: 10, ease: 'easeOut' },
      ]),
    ).toMatchObject({ property: 'cx' });
  });
});
