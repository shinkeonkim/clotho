...
> 다만 정정할 것이 있다. 처음에 "재생이 멈춰 있다"를 React 버그로 진단했으나, 실제
> 원인은 **자동화 탭이 백그라운드라 Chrome이 rAF를 정지**시킨 것이었다
> (`visibilityState: 'hidden'`, rAF 발화 0회). React 결함 두 가지는 그 자체로 실재하고
> 고칠 값어치가 있지만 그 증상의 원인은 아니었다.
...
**9.1의 요점은 갤러리가 썩지 않는다는 것이다.** `tests/gallery-coverage.test.ts`가
스키마를 역으로 조회해서 커버리지를 강제한다 — 열거값이 하나 늘면 갤러리가 그것을
그릴 때까지 빌드가 깨진다. 문서 파싱·검증·씬 구축·SVG 직렬화, 그리고 캔버스 경계
안에 있는지까지 41개 테스트로 확인한다.

**9.2가 찾은 것.** 에디터 → clotho 방향은 100%다(회귀 없음). 반대 방향에 구멍이 있다:
`code` 요소는 에디터에서 **만들 수조차 없고**, 키프레임 `ease`와 v1 신규 `interpolate`는
UI가 전혀 없다. 속성 패널이 그리지 않는 필드가 37개다. 조치 목록 A1~A10을 문서에 남겼다.

**9.4의 근거.** 실데이터 383개 사용 패턴, 9.2가 드러낸 구멍, 그리고 갤러리를 만들며
실제로 겪은 불편(P1 반복 트랙이 여기서 나왔다 — `iteration` 문서를 손으로 쓸 수 없어서
TypeScript로 생성했다).
...
배포 전까지 소비처와 clotho-editor는 `file:` 로컬 링크로 동작한다. 배포 후 버전 지정으로
바꾸면 커밋 가능한 상태가 된다.
