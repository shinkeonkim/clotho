...
기능 14개를 한 번에 낸다. 요소나 효과가 아니라 **문서를 둘러싼 것들** — 저작 루프, 내보내는 자리, 끼워 넣는 자리 — 이 대부분이다.

### 추가 — 문서가 표현하는 것

- **`charts` (§2.15)**: 저작 시간 스펙이며 `compileCharts`가 평범한 v1 요소로 낮춘다. **런타임은 차트를 전혀 모른다** — 어댑터 4종·GIF·에디터의 요소 switch 어디에도 12번째 타입이 생기지 않고 렌더 경로 비용이 0이다. 설계의 전부는 **예측 가능한 id**(`bench__series-quick__point-3`)이고, 그래서 차트를 위한 특별한 강조 문법이 필요 없다 — 기존 `pulse`·`spotlight`·`camera.focus`가 그대로 통한다. `kind`는 `bar`와 `line` 둘뿐이며 이 목록은 좁게 유지한다.
- **`style` (§2.16)**: 무엇을 그리는지가 아니라 **어떻게 그리는지**를 정한다. `clean`(기본, 출력이 바이트 단위로 동일) · `sketch` · `mono`. `buildScene` 이후 어댑터 이전의 **순수 Scene → Scene 변환**이라 요소 빌더 11종을 건드리지 않고 어댑터 4종과 GIF가 공짜로 따라온다. 지터는 결정적이다 — 시드는 `hash(seed, node.key, node.kind)`이고 **시각 `t`는 시드에 들어가지 않는다.** 넣으면 매 프레임 선이 다시 섞여 그림이 끓는다.
- **`code.source` (§2.17)**: `content`가 어디서 왔는지를 기록한다. `region`은 위에 줄이 추가돼도 살아남고 `lines`는 밀리는 순간 조용히 틀린 곳을 가리킨다 — region이 권장 기본값인 이유다. **런타임은 파일을 읽지 않는다**; `content`가 언제나 진실이다. 이름이 같은 region이 둘이면 앞의 것을 조용히 고르지 않고 **거부한다.**
- **`chapters[].notes` (§2.18)**: 발표자 메모. `subtitle`은 청중이 읽는 자막이라 겸용할 수 없다 — 공유 화면에 나오는 필드가 "내가 무슨 말을 하려 했더라"를 적는 자리가 될 수는 없다.
- **track property 검증**: 스키마에 없는 속성을 트랙이 지목하면 경고한다. 파서는 미지의 키를 조용히 버리므로 지금까지는 아무 일도 일어나지 않았다.
- **`bakeMathElements`**: math 요소를 저작 시간에 글리프 path로 굽는다. 조판기 없는 환경에서도 수식이 수식으로 남는다.

### 추가 — 저작 루프

- **`clotho dev`**: 디렉터리를 띄우고 파일이 밖에서 바뀌면 **재생 위치를 유지한 채** 다시 그린다. 검증·린트를 같은 화면에 띄운다. 브라우저는 임의의 로컬 디렉터리를 열 수 없으므로 **에디터가 채울 수 없는 자리**다. 파일을 쓰는 엔드포인트가 있어 기본 바인딩이 `127.0.0.1`이다.
- **`clotho diff`**: JSON 텍스트 diff로는 애니메이션 변경을 읽을 수 없다 — 요소 하나를 추가하면 수백 줄이 움직인다. 저작자의 단위로 보고하고, id만 바뀐 경우는 삭제+추가가 아니라 **rename으로 추정**해 보여준다(추정임을 표시한다).
- **`clotho explain`**: 저작 중 실제로 나오는 질문은 늘 같다 — **이 요소가 왜 안 보이지.** 가시성 사유를 먼저, 그리고 **하나만 찾고 멈추지 않는다**: 등장 구간 밖이면서 동시에 숨겨진 그룹 안일 수 있고 한쪽만 고치면 여전히 안 보인다.
- **`clotho storyboard`**: 컨택트 시트 PNG 또는 프레임별 파일. 프레임은 기본적으로 **챕터**에서 고른다 — 저작자가 이미 단계를 나눠 놓았으므로 어떤 휴리스틱보다 낫다.

### 추가 — 끼워 넣는 자리

- **`@kokoa/clotho/mdx`**: 빌드 시 문서를 검증하고(실패하면 빌드가 실패한다) 포스터 프레임 SVG를 인라인한다. JS 없이도 그림이 보인다. 프레임워크 컴포넌트가 아니라 **HTML**을 내보내므로 Astro·Next·Docusaurus·Vitepress가 같은 경로를 탄다.
- **`@kokoa/clotho/element`**: `<clotho-player src=… >`. shadow root에 렌더하고 **스타일시트를 함께 들고 간다** — 남의 페이지에서 호스트 CSS와 충돌하지 않는다. 테마 토큰은 CSS 변수라 경계를 통과한다.
- **딥링크** `bindUrlState`·`shareUrl`: 시각(`t`)보다 **챕터(`c`)를 권장**한다 — 문서를 고치면 `t=3200`은 어긋나지만 챕터 id는 저작자가 이름 붙인 그 순간을 계속 가리킨다. `replaceState`를 쓰므로 뒤로 가기가 애니메이션 히스토리로 채워지지 않는다.
- **스크롤리텔링** `mountScrollPlayer`: 시계를 스크롤로 갈아끼운다. `seek(t)`가 순수하기 때문에 되감기가 자유롭다. `prefers-reduced-motion`에서는 **챕터별 정지 프레임으로 강등**한다 — 스크롤 하이재킹은 전정기관 문제가 있는 독자에게 특히 나쁘고 속도를 늦추는 것으로 해결되지 않는다.
- **발표자 모드** `mountPresenter`: 챕터를 **구간**으로 다뤄 다음을 누르면 그 구간을 **재생하고 멈춘다.** 챕터 시각으로 점프하지 않는다 — 애니메이션이 설명의 일부라서 건너뛰면 청중은 과정 없는 결과만 본다.

### 호환성

**완전 하위호환.** 문서에 추가된 필드(`charts`·`style`·`code.source`·`chapters[].notes`)는 전부 선택적이고, 기존 문서의 출력은 visual baseline 144프레임이 증명하듯 바이트 단위로 동일하다. 진입점 `./mdx`와 `./element`가 늘었다.
